...
12. **`--feature-profile raw|core` added** (mirrors sibling pattern commands): `core` restricts feature patterns to pos1–pos4, making full-corpus runs far cheaper; recorded in the `score_version` block as `feature_profile` since it changes which patterns exist.
