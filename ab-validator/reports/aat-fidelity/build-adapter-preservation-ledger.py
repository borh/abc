...
matrix = {r["id"]: r for r in tomllib.load(open("data/aozora-syntax-coverage.toml", "rb"))["syntax"]}
...
for r in rows:
    for sid in c2s.get(r["case_id"], []):
        cased.add(sid)
        a = agg[sid][r["adapter"]]; a[1] += 1; a[0] += (r["oracle_status"] == "pass")

def occ(row): return (row.get("corpus_prevalence") or {}).get("total_occurrences") or 0
...
for sid, row in matrix.items():
    ...
    ledger.append({
        "id": sid, "category": row.get("category"),
        "representability": (row.get("representability") or {}).get("status"),
        "occ": occ(row), "works": prev.get("works_with_feature"),
        "has_case": sid in cased,
        "matrix_fidelity": mfid, "live": live,
...
summary = {
    "constructs": n, "adapters": ADS,
    "verified_preserved": len(verified),
    "uncovered": len(uncovered),
    "uncovered_nonzero_occ": len(uncovered_occ),
    "stale_cells": stale,
    "no_adapter_preserves_with_case": sum(1 for x in ledger if x["has_case"] and not x["preserved_live"]),
}
...
def key(x):
    ...
    return (grp, -x["occ"])
ledger.sort(key=key)
json.dump({"summary": summary, "ledger": ledger}, open(sp / "corrected-ledger.json", "w"),
          ensure_ascii=False, indent=1)
print(json.dumps(summary, indent=2))
print("\nstale cells (adapter live-passes but matrix != preserved):", stale)
print("uncovered constructs:", [x["id"] for x in uncovered], "(all occ:", [x["occ"] for x in uncovered], ")")
