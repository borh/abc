...
rm -rf "$out"
mkdir -p "$run/check-reports/fixture-adapter" "$run/aat/fixture-adapter" "$audit" "$samples"
...
cat > "$run/index.json" <<'JSON'
{
  ...
  "works": [],
  "by_feature": {
    "warigaki": ["work-war-ok", "work-war-missing"],
    "kaeriten": ["work-kun-ok", "work-kun-timeout"],
    "okurigana": []
  }
}
JSON
...
jq -e '.sample_counts.warigaki.observed_in_aat == 1' "$samples/policy-samples.summary.json"
jq -e '.sample_counts.warigaki.source_feature_without_aat_observation == 1' "$samples/policy-samples.summary.json"
jq -e '.sample_counts.kunten.observed_in_aat == 1' "$samples/policy-samples.summary.json"
