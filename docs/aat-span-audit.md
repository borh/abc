...
| `adapters/aozora2html/src/lib.rs` | AAT nodes | No current AAT source-span emission found. XHTML `<span>` elements are parsed as markup nodes, not AAT coordinate spans. | Search hits `span` as an XHTML element name, not AAT `span` coordinate fields. |
