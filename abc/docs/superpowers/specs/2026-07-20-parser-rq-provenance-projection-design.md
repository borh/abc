...
correctly remains Proposed because the frozen promotion verifier rejects the
...
`verify-provenance-errors` separately requires a bound record to have the exact
v2 `schema_id`, `schema_version`, and closed key set. Projection answers whether
two representations contain the same candidate evidence; envelope validation
answers whether the supplied bound representation is a valid protocol value.
Neither check substitutes for the other.

## Authorization Boundary

`verify-authorization-record` must include `verify-provenance-errors`. A capture
authorization is structurally valid only when:
...
committed and before the one-shot capture begins. Promotion retains the same
check as defense in depth.

## Failure Semantics

Any candidate/provenance projection mismatch makes authorization invalid. The
operator must not rewrite the candidate or provenance record. The candidate is
abandoned, the contract defect is fixed in a later revision, and a new
candidate is frozen.

The completed candidate at
`sha256:15affdfb677cc6a94a4a5364da68ca2d11441f899737651e727dbac90eddc5ab`
remains immutable and non-promotable. This change does not reinterpret its
authorization, capture, evaluation, or report.
...
It does not retry or amend the completed P5 candidate. A new authoritative
campaign, if desired, begins only after the correction is implemented and all
pre-freeze checks pass.
...
- authorization cannot call the existing provenance verifier without creating
