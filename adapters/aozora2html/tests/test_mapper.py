...
def test_nested_paragraphs_are_source_derived_normalized() -> None:
    sys.path.insert(0, str(REPO_ROOT))
    from adapters.aozora2html.adapter import normalize_source_derived_blocks
    ...
    assert normalize_source_derived_blocks(blocks, {}, "full source") == [
        {
            "kind": "jisage_block",
            "children": [
                {
...
def test_split_gaiji_ruby_notes_are_source_derived_in_nested_paragraphs() -> None:
    sys.path.insert(0, str(REPO_ROOT))
    from adapters.aozora2html.adapter import normalize_source_derived_blocks
...
def test_inlined_resolved_gaiji_text_is_source_derived() -> None:
    sys.path.insert(0, str(REPO_ROOT))
    from adapters.aozora2html.adapter import normalize_source_derived_blocks

    source = "芒の快い刺※［＃「卓＋戈」、U+39B8、32-上-8］を感じた。"
    blocks = [
        {
            "kind": "paragraph",
            "content": [{"kind": "text", "value": "芒の快い刺㦸を感じた。"}],
        }
    ]

    assert normalize_source_derived_blocks(blocks, {}, source) == [
        {
            "kind": "paragraph",
            "content": [
...
def test_inlined_resolved_gaiji_text_is_source_derived_in_mixed_content() -> None:
    sys.path.insert(0, str(REPO_ROOT))
    from adapters.aozora2html.adapter import normalize_source_derived_blocks

    source = "經濟に刺※［＃「卓＋戈」、U+39B8、64-上-17］されてその人《にん》"
    blocks = [
        {
            "kind": "paragraph",
            "content": [
...
def test_inlined_resolved_gaiji_ruby_base_is_source_derived() -> None:
    sys.path.insert(0, str(REPO_ROOT))
    from adapters.aozora2html.adapter import normalize_source_derived_blocks

    source = "沙漠の砂《すな》の※［＃「檄」の「木」に代えて「火」、U+71E9、35-3］《や》けて"
    blocks = [
        {
            "kind": "paragraph",
            "content": [
...
def test_unresolved_gaiji_placeholder_in_ruby_base_is_source_derived() -> None:
    sys.path.insert(0, str(REPO_ROOT))
    from adapters.aozora2html.adapter import normalize_source_derived_blocks
