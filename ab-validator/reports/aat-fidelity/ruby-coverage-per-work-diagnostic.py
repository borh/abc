...
def rubycount(path):
    try:
        with open(path) as handle:
            blocks = json.load(handle).get("blocks", [])
    except Exception:
        return None
    c = [0]
    st = [blocks]
    while st:
        ...
        if isinstance(v, dict):
            k = v.get("kind")
            if k == "ruby" or v.get("x-source-marker-kind") == "ruby":
                c[0] += 1
            st.extend(v.values())
...
a2 = sum(per["aozora2html"][w] for w in sh)
rs = sum(per["aozora-rs"][w] for w in sh)
...
print(
    f"  works aozora-rs has but a2html MISSING: {len(missing)} works, {mruby:,} ruby ({100 * mruby / rs:.1f}% of rs ruby)"
)
...
print(
    f"  per-work ratio (shared, rs>0): median={statistics.median(ratios):.3f} mean={statistics.mean(ratios):.3f}"
)
