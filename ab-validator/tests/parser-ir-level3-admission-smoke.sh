...
cat > "$matrix_summary" <<JSON
{
  ...
  "rows": [
    ...
    {
      "work_id": "plain-raw-only",
      "title": "Plain raw only adapter residue",
      "tei_eaj_file": "data/plain-raw-only.xml",
      "selected_aat": {"adapter": "aozora", "adapter_version": "fixture"},
      "materialization": {"status": "passed"},
      "tei_eaj": {"structure_profile": "plain_prose"},
...
jq -e '.mapping.mapping_version == "0.2.4"' "$summary_json"
jq -e '.mapping.mapping_hash | test("^sha256:[0-9a-f]{64}$")' "$summary_json"
jq -e '.mapping.mapping_schema_hash == "sha256:23a2822cbae88533168121e8a09648441276d8af6484269ae666b90030eb1e06"' "$summary_json"
jq -e '.mapping.target_parser_ir_schema_id == "https://w3id.org/abc/schemas/parser-ir.schema.json"' "$summary_json"
jq -e '.mapping.target_parser_ir_schema_hash == "sha256:0ab6f07e681b7adb14b9cacb14e4f406ef122151df4d1554503e77a3f1faf8c2"' "$summary_json"
...
jq -e '.plain_prose_admission.paragraph_origin_buckets.adapter_raw_only == 1' "$summary_json"
