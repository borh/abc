...
logical `artifact_root` values contain no host-local path.
...
The resolver also supplies inventory identities and one materialization
attestation per execution. Each attestation binds candidate/mode, frozen parser
and adapter revisions, executable byte hash, and Nix derivation or workspace
build identity. The checked summary stores only its canonical SHA-256. The
generator consumes the preregistration directly, hashes and counts both
inventory files, derives all frozen scalar/revision fields, derives every run
hash/count from external artifacts, and only then compares regenerated bytes.
