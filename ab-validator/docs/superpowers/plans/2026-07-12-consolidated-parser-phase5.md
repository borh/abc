...
- Corpus-bound expectations — **PARSER-VISIBLE universe (amendment 3, Task 9 rerun reconciliation)**: adopted yokogumi pairs **1552**; adopted keigakomi pairs **25**; declined raw-preserved markers **14** (all reopen-rollback: the `000094_42338` line); reasons {orphan_open: 0, orphan_close: 0, reopen_rollback: 14, interleave: 0}. The placement report's SOURCE-TEXT figures (1582/25/24) remain true of the source text; 70 source-text markers are by design never parser-visible as standalone raw marker nodes: 44 inside the notation-legend block (incl. all 10 orphan opens — the （例） example lines), 12 inside the Phase 4 source_note tail region, 14 inside one work's merged unparsed-source-gap raw node (`001111_42789`). The delta gate binds the parser-visible figures; the reconciliation is documented in the placement report's Revision 4 section and independently re-derived before rebinding.
