...
**Goal:** Build the first reusable alignment-probe slice: a spanless token-sequence alignment kernel, a standalone ABC alignment-probe schema, and a Rust TEI-EAJ Melos characterization adapter.
...
**Tech Stack:** Rust 2024 workspace crates, `serde`, `serde_json`, `proptest`, JSON Schema draft 2020-12, ABC Clojure schema tests via `m3.json-schema`, Nix flake checks.
...
- Create: `abc/fixtures/alignment-probe/bad-diagnosis-count.semantic-invalid.json`
...
- [ ] **Step 2: Export the align module**
...
- [ ] **Step 3: Write failing kernel tests**
...
pub fn algorithm_config_hash(config: &AlignmentConfig) -> String {
    ...
    crate::hashing::hash_bytes(canonical_json_string(&value).as_bytes())
}
...
This canonicalizer is deliberately local to `AlignmentConfig`: that value
contains objects, arrays, strings, booleans, nulls, and integers only. Do not
reuse it for arbitrary manifest identity values with floating-point numbers.

- [ ] **Step 4: Run the failing kernel tests**
...
- [ ] **Step 5: Implement the minimal deterministic kernel**
...
- [ ] **Step 6: Run kernel tests to green**
...
fn exact_unmatched_sequence_can_be_marked_likely_moved() {
    let left = vec![tok(0, "A"), tok(1, "B"), tok(2, "C"), tok(3, "D")];
    let right = vec![tok(0, "A"), tok(1, "D"), tok(2, "B"), tok(3, "C")];
    let result = align_pair(&left, &right, &AlignmentConfig::default());
    assert!(result
        .regions
        .iter()
...
Create `abc/fixtures/alignment-probe/bad-diagnosis-count.semantic-invalid.json` by copying the valid fixture and changing:
...
(deftest alignment-probe-schema-fixtures-validate-test
  (let [probe-schema (schema/read-schema "schemas/alignment-probe-v1.schema.json")
        ...
        bad-count (files/read-json "fixtures/alignment-probe/bad-diagnosis-count.semantic-invalid.json")
    ...
    (is (not= (get bad-count "diagnosis_event_count")
              (reduce + (vals (get bad-count "diagnosis_counts")))))))
```
...
git add abc/schemas/alignment-probe-v1.schema.json \
        ...
        abc/fixtures/alignment-probe/bad-diagnosis-count.semantic-invalid.json \
...
- [ ] **Step 3: Add failing Melos adapter test**
...
fn tei_eaj_alignment_probe_classifies_melos_tail_addition() {
    ...
    std::fs::write(
        &abc_tei,
        r#"<TEI><text><body><p>勇者は、ひどく赤面した。</p><p>（古伝説と、シルレルの詩から。）</p></body></text></TEI>"#,
    )
    .unwrap();
    std::fs::write(
        &tei_eaj_file,
        r#"<TEI><text><body><p>勇者は、ひどく赤面した。</p></body></text></TEI>"#,
    )
    .unwrap();
    std::fs::write(
        &workset_path,
  ...
  "files": [{{
    ...
    "abc_body_base_text_length": 31,
    "tei_eaj_body_base_text_length": 15,
    "abc_p_count": 2,
    "tei_eaj_p_count": 1,
    "abc_note_count": 0,
...
- [ ] **Step 4: Run the failing adapter test**
...
- [ ] **Step 5: Implement adapter data model**
...
Add private deserialization structs for the workset fields used by the adapter: `tei_eaj_source.root`, `files[].work_id`, `files[].title`, `files[].tei_eaj_file`, `files[].abc_tei`, `files[].comparison_status`, and `files[].base_text_equal`.

- [ ] **Step 6: Implement TEI token extraction**

Use `tl::parse` to extract body paragraphs from TEI. Add:
...
fn extract_body_tokens(xml: &str) -> Result<Vec<TeiToken>> {
    let dom = tl::parse(xml, tl::ParserOptions::default()).context("failed to parse TEI XML")?;
    let parser = dom.parser();
    let mut tokens = Vec::new();
    let body = dom
        .nodes()
        .iter()
        .find(|node| node.as_tag().is_some_and(|tag| tag.name().as_utf8_str().eq_ignore_ascii_case("body")))
        .context("TEI body not found")?;
    collect_text_tokens(body, parser, "/TEI/text/body".to_owned(), &mut tokens);
    Ok(tokens)
}
```

Implement `collect_text_tokens` so it prefers direct `<p>` and `<head>` descendants as structural tokens. For each token:

- `text` is visible body text.
- `normalized` is `text` with all Unicode whitespace removed.
- `features` contains `"source-attribution"` when normalized text starts with `（` and contains `シルレルの詩から`.
- `path` is a stable best-effort path such as `/TEI/text/body/p[2]`.

- [ ] **Step 7: Implement probe construction**
...
pub fn run_tei_eaj_alignment_probe(
    ...
    let workset: Workset = serde_json::from_slice(&std::fs::read(&config.workset_path)?)
        .context("failed to parse TEI-EAJ workset JSON")?;
    let root = PathBuf::from(workset.tei_eaj_source.root);
    let mut rows = Vec::new();
    for file in workset.files.into_iter() {
        ...
        rows.push(TeiEajAlignmentProbeRow {
            ...
            title: file.title,
            tei_eaj_file: file.tei_eaj_file,
...
`build_probe` must call `align_pair`, map kernel regions to samples, and classify a final insertion whose left context has `"source-attribution"` as `tail_addition`.

- [ ] **Step 8: Run the adapter test to green**
...
Define `minimal_melos_alignment_workset` next to the existing integration test helpers. It should create the same temporary ABC and TEI-EAJ XML files as Task 4 and return the workset JSON string.
...
- [ ] **Step 6: Generate checked-in Melos characterization report**

Run from repository root:

```bash
nix run .#ab-validator-ab-aat-to-parser-ir -- tei-eaj-alignment-probe \
  --workset abc/docs/handoffs/tei-eaj-aozora-workset-export.json \
...
- JSON report has rows for the two Melos Level 4 TEI-EAJ files.
- Each row has `alignment_probe.diagnosis_counts.tail_addition = 1`.
- Markdown report contains `tail_addition` and `1567_tei.xml`.
...
- Config hash and config object: Task 1 hash function, Task 3 schema/fixture, Task 4 probe output.
- Diagnosis count invariant: Task 3 schema tests.
- Rust-only first consumption surface: Tasks 4 and 5; Python integration is not touched.
- Melos baseline characterization: Tasks 4 and 5.
