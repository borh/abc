...
This rights statement is identified by <https://w3id.org/soranoha/rights>.
Every signed release manifest names this identifier in `rights.statement_url`,
and every published TEI file references it in `publicationStmt/availability` via
`licence/ptr/@target`. Because signed bytes cannot be modified later, they record
this persistent identifier rather than a transient host URL. Registering the
`w3id.org/soranoha/` redirects is a precondition of initial publication; the
identifiers are written here in the form the signed bytes name.
...
Every published work combines two distinct rights components:

**The underlying works** are texts from [Aozora
Bunko](https://www.aozora.gr.jp/), under one of two standings. Almost all of
them are works whose copyright term has expired. Soranoha neither holds nor
claims any right in those. Aozora Bunko's [handling rules](https://www.aozora.gr.jp/guide/kijyunn.html)
permit free copying, redistribution, performance, and adaptation of
copyright-expired files, with no permission or fee required. Altering the
text itself is permitted within the scope of Article 20(2)(iv) of the Copyright
Act, which covers format conversion, notation changes, and modernising
旧字旧仮名.
...
bibliographic data derives from Aozora Bunko's catalog CSV under CC-BY-4.0.
Redistributing the catalog or listings built from it requires attributing Aozora
Bunko as the source.
...
| Data tables | Per-file source provenance and terms | `ab-validator/crates/*/data/` |
| Aozora Bunko catalog CSV (bibliographic metadata) | CC-BY-4.0 | Upstream metadata source |
...
in each file header. The gaiji annotation dictionary (`外字注記辞書`) mapping is CC0.
The JIS X 0213 code tables follow Project X0213's permissive terms. Gaiji alias mappings
are locally authored. The Aozora Bunko catalog CSV used for bibliographic metadata
is published by Aozora Bunko under CC BY 4.0.
...
and, beside each work, the standing that work was admitted under is recorded as:
...
Both representations originate in `soranoha/data/publication-policy.edn`, hashed into each
...
rejects files whose `publicationStmt/availability/licence` lacks a `@target`.
...
- The work is removed from `works[]`, so its artifacts are no longer served at
  ...
  longer describes it: a manifest may not name a catalog describing a work
  withdrawn by that release.
...
See [`design/snh-protocol-v1.md`](design/snh-protocol-v1.md) and
