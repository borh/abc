(ns abc.tools.parser-rq-diagnostic-completeness
...
(defn- projected-hash
  ...
  (hash/format-sha256
   (hash/sha256-json-jcs
    (json/read-json-str
     (json/write-deterministic-json-str (dissoc value field))))))
...
(defn- validation-result
  [bytes]
  (try
    (let [value (json/read-json-str (strict-utf8 bytes))
...
(defn- ledger-blob
  [errors]
  (let [bytes (.getBytes (str (json/write-deterministic-json-str
                               {:errors errors}) "\n")
