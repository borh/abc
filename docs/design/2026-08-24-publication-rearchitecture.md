...
| D23 | **Aozora reliance as the default admission basis.** Accept Aozora Bunko's published copyright-expired classification for the identified work/edition, scoped to Japan; retain the source assertion, applicable usage rules, source revision and observation date. Soranoha records its own decision to rely on that assertion; it does not fabricate independent contributor assessments or a guarantee. Independent rights reconstruction is reserved for concrete contradictory evidence or restrictions. Mere presence in the Git mirror is insufficient because removed public-site files can remain there. This supersedes O1/D21 and D-3/D-4 insofar as they require universal independent assessment or forbid source reliance. Their statutory evidence and independent-assessment derivations remain. No worldwide claim, protected/permission-only admission, schema alteration, policy activation or genesis is authorized by this decision alone. | owner-adopted 2026-09-06, decision interview | implemented under D24; production reliance acceptance remains outstanding |
| D24 | **D23 implementation amendment: versioned edition-reliance evidence.** Add `snh-assessment-snapshot/2` as an explicit schema discriminator under the existing `assessment-snapshot` artifact type. Version 2 permits an edition-reliance candidate instead of independent work/contribution facts; it records Aozora attribution, Japan scope, the retained observed classification, evidence digests, source revision, observation date and Soranoha decision date/basis, with current applicability and exception reason. Version 1 schema bytes, vectors and behavior remain supported unchanged; no reliance records still emits version 1. The new inclusion rule admits applicable reliance or the existing unanimous independent assessment route and otherwise fails closed. Manifest/report shape and hash encodings remain unchanged; old readers reject version 2. The assembler checks reliance content identity against built content. Raw supporting evidence remains internal, outside public artifact resolution; digests identify retained evidence, not public blob promises. | implementation of owner-authorized D23, recorded before schema/code changes | pre-publication D16 amendment; conformance and verifier/serving checks required; no publication activation |
| D25 | **Greenfield assessment contract: version 2 only.** The owner explicitly withdraws legacy compatibility requirements. Emit and accept only `snh-assessment-snapshot/2`, including empty and independent-assessment-only snapshots; use its one current inclusion rule and always verify the admission partition. Retire version-1 runtime/schema/vector routing and obsolete scaffolding/ABC parity entry points. Independent findings and edition reliance remain current domain alternatives. Historical research records remain evidence, not supported runtime inputs. Manifest/signature encodings and current trust invariants are unchanged. | owner-directed 2026-09-06 greenfield audit, recorded before protocol asset changes | supersedes D24 backward-compatibility clauses; no publication or policy activation |
