...
SKIP_BASE_TEXT_LOCALS = {"rt", "rp", "note"}
SKIP_BASE_TEXT_SPAN_TYPES = {"rt", "rp"}
SKIP_BASE_TEXT_SPAN_RENDS = {"notes"}
SPAN_FEATURE_TYPES = {"ruby", "rb", "rt", "rp"}
COUNTERPART_TABLE_LIMIT = 100
REPORT_FEATURES = [
    "p",
    "s",
    "note",
...
def skip_text_node(node, skip_locals):
    name = local_name(node.tag)
    if name in skip_locals:
        return True
    if skip_locals and name == "span":
        span_type = node.attrib.get("type")
        span_rend = node.attrib.get("rend")
        return span_type in SKIP_BASE_TEXT_SPAN_TYPES or span_rend in SKIP_BASE_TEXT_SPAN_RENDS
    return False
...
def element_text(element, skip_locals=frozenset()):
    ...
    def visit(node):
        if skip_text_node(node, skip_locals):
            return
        if node.text:
...
def analyze_file(path):
    ...
    for element in root.iter():
        if local_name(element.tag) == "span" and element.attrib.get("type") in SPAN_FEATURE_TYPES:
            counts[element.attrib["type"]] += 1
...
def unique_title_work_id(title, title_to_work_ids=None):
    if not title or not title_to_work_ids:
        return None
    work_ids = title_to_work_ids.get(normalize_ws(title), set())
    if len(work_ids) == 1:
        return next(iter(work_ids))
    return None


def tei_eaj_work_identity(relative_path, title=None, title_to_work_ids=None):
    stem = pathlib.PurePosixPath(relative_path).stem
    match = re.fullmatch(r"(\d+)_tei", stem)
    if match:
        candidate = canonical_work_id(match.group(1))
        title_work_id = unique_title_work_id(title, title_to_work_ids)
        if title_work_id and title_work_id != candidate:
            return title_work_id, "title_unique_index_correction"
        return candidate, "filename_work_id"
    match = re.fullmatch(r"(\d+)_header.*", stem)
    if match:
        return canonical_work_id(match.group(1)), "filename_header_work_id"
    match = re.fullmatch(r"(\d+)-\d+_tei", stem)
    if match:
        return canonical_work_id(match.group(1)), "filename_work_id_variant"
    match = re.fullmatch(r"\d+_(\d+)", stem)
    if match:
        return canonical_work_id(stem.split("_", 1)[0]), "filename_work_id_file_id"
    match = re.fullmatch(r"(\d{3,})", stem)
    if match:
        return canonical_work_id(match.group(1)), "filename_numeric_work_id"
    return None, "no_work_id"


def tei_eaj_work_id(relative_path, title=None, title_to_work_ids=None):
    work_id, _method = tei_eaj_work_identity(relative_path, title, title_to_work_ids)
    return work_id


def discover_tei_eaj_files(source_root, title_to_work_ids=None):
    ...
    for path in sorted(data_root.rglob("*.xml")):
        ...
        work_id, work_id_method = tei_eaj_work_identity(
            relative_path, features["title"], title_to_work_ids
        )
        records.append(
            {
                "path": str(path),
...
def compare_features(abc_features, record):
    ...
    relation = base_text_relation(abc_text, other_text)
    return {
        "relpath": record["relpath"],
        "work_id": record.get("work_id"),
        "work_id_method": record.get("work_id_method"),
        "state": record["state"],
        "level": record["level"],
        "title": record["title"],
        "base_text_equal": abc_text == other_text,
        "base_text_relation": relation,
        "base_text_length_delta": len(abc_text) - len(other_text),
...
def base_text_relation(abc_text, tei_eaj_text):
    if abc_text == tei_eaj_text:
        return "equal"
    if tei_eaj_text and tei_eaj_text in abc_text:
        return "tei_eaj_subset_of_abc"
    if abc_text and abc_text in tei_eaj_text:
        return "abc_subset_of_tei_eaj"
    return "mismatch"
...
def inspection_index_path(directory):
    directory = pathlib.Path(directory)
    candidate = directory.parent / "inspection-index.tsv"
    return candidate if candidate.exists() else None


def read_inspection_index_counterparts(directory):
    index_path = inspection_index_path(directory)
    ...
    with index_path.open(encoding="utf-8", newline="") as handle:
        ...
        for row in reader:
            work_id = canonical_work_id(row.get("work_id", ""))
            if not work_id:
                continue
            path = pathlib.Path(directory) / f"{int(work_id):06d}.xml"
            if not path.exists():
                continue
            counterparts[work_id] = {
                "work_id": work_id,
                "path": path.as_posix(),
...
def counterpart_features(counterpart):
    if counterpart.get("features") is None:
        counterpart["features"] = analyze_file(counterpart["path"])
    return counterpart["features"]
...
def discover_abc_counterparts(abc_tei_specs, abc_tei_dirs):
    ...
    for spec in abc_tei_specs:
        ...
        if counterpart and counterpart["work_id"]:
            counterpart["title"] = counterpart["features"].get("title")
    ...
    for directory in abc_tei_dirs:
        ...
        if not directory.exists():
            continue
        indexed = read_inspection_index_counterparts(directory)
        if indexed:
            for work_id, counterpart in indexed.items():
                counterparts.setdefault(work_id, counterpart)
            continue
...
def counterpart_title_index(counterparts):
    title_to_work_ids = collections.defaultdict(set)
    for work_id, counterpart in counterparts.items():
        title = counterpart.get("title")
        if not title and counterpart.get("features"):
            title = counterpart["features"].get("title")
        if title:
            title_to_work_ids[normalize_ws(title)].add(work_id)
    return title_to_work_ids
...
def all_work_comparison_row(counterparts, record):
    ...
    if work_id is None:
        return {
            ...
            "base_text_relation": None,
            "base_text_length_delta": None,
    ...
    comparison = compare_features(counterpart_features(counterpart), record)
...
def build_all_work_report(abc_tei_specs, abc_tei_dirs, source_root, source_rev=None):
    ...
    records = discover_tei_eaj_files(source_root, counterpart_title_index(counterparts))
    ...
    relation_counts = collections.Counter(row["base_text_relation"] for row in compared_rows)
    return {
        ...
        "base_text_relation_counts": dict(sorted(relation_counts.items())),
    }
...
def render_all_work_markdown(report):
    ...
    if len(counterpart_rows) > COUNTERPART_TABLE_LIMIT:
        lines.append(
            f"ABC counterpart table omitted: {len(counterpart_rows)} discovered counterparts "
            f"exceeds display limit {COUNTERPART_TABLE_LIMIT}."
        )
    elif counterpart_rows:
    ...
    for row in report["all_work_rows"]:
        ...
        comparison_rows.append(
            [
                ...
                p_counts.get("ab-validator/research"),
                p_counts.get("tei_eaj"),
                note_counts.get("ab-validator/research"),
...
def workset_export_row(row):
    ...
    return {
        "work_id": row["work_id"],
        "work_id_method": row.get("work_id_method"),
        ...
        "base_text_relation": (
            row["base_text_relation"]
            if row["base_text_relation"] is not None
            else row["missing_reason"]
            if row["missing_reason"] == "missing_abc_counterpart"
            else None
        ),
        "base_text_length_delta": row.get("base_text_length_delta"),
        "abc_body_base_text_length": row["abc_body_base_text_length"],
        "tei_eaj_body_base_text_length": row["tei_eaj_body_base_text_length"],
        "abc_p_count": p_counts.get("ab-validator/research"),
        "tei_eaj_p_count": p_counts.get("tei_eaj"),
        "abc_note_count": note_counts.get("ab-validator/research"),
...
def workset_export(report):
    ...
    return {
        ...
        "summary": {
            ...
            "base_text_relation_counts": report["base_text_relation_counts"],
        },
