...
(defn- font-family [theme]
  (or (:font-family theme) "Noto Sans CJK JP"))

(defn- canvas-color
  "Presentation figures default to a black canvas (the palette's off-white
  text depends on it); transparency must be an explicit :transparent."
  [theme]
  (let [canvas (:canvas theme)]
    (cond
...
(defn dot [{:keys [id direction theme groups nodes edges primary-order
  ...
  (let [grouped (group-by :group nodes)
         ...
         (fn [{:keys [id label style label-location]}]
           (concat
            [(str "  subgraph \"cluster_" (id-string id) "\" {")
             ...
             (str "    fontname=\"" (escape-dot (font-family theme)) "\";")
    ...
    (str
     "// GENERATED by abc.tools.diagram.graphviz/dot — do not edit\n"
     "digraph \"" (id-string id) "\" {\n"
     "  graph " (attrs (cond-> {:bgcolor (canvas-color theme)
                                :fontname (font-family theme)
     ...
     ";\n"
     "  node " (attrs {:fontname (font-family theme)
                       :margin "0.04,0.02"})
     ";\n"
