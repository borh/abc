...
- **ADR-0023-C1 — structural-invariant:** The AAT mapping, divergence, and divergence-bundle schemas are valid Draft 2020-12 schemas under the real meta-schema validator. Evidence: `test/abc/tools/validate_design_bundle_test.clj`.
- **ADR-0023-C2 — structural-invariant:** The committed compatibility registry requires the complete adapter/version/mapping/parser-IR match key, matching evidence scope, and no wildcard adapter entries. Evidence: `test/abc/tools/aat_parser_ir_compat_test.clj` and `test/abc/tools/validate_design_bundle_test.clj`.
- **ADR-0023-C3 — fixture-behavior:** The admission boundary reports admitted, missing, and conflict states and treats changed evidence under an existing match key as conflict. Evidence: `test/abc/tools/validate_design_bundle_test.clj`.
- **ADR-0023-C4 — fixture-behavior:** Import materialization copies `mapping_hash` into `manifest_identity_object.aat_parser_ir_mapping_hash`. Evidence: `test/abc/tools/materialize_import_test.clj`.
- **ADR-0023-C5 — operational-behavior:** The supported design-bundle application exits zero over the committed mapping schemas, registry, and imported fixture. Evidence boundary: `test/abc/tools/validate_design_bundle_test.clj`.
