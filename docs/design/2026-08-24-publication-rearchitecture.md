...
| D13 | Identifiers ratified: artifact id = typed content hash; release name = `r<date>-<manifesthash12>`; snapshot DOIs **quarterly** (Zenodo concept DOI + version DOIs). Prefix harmonized with 2026-07-03 naming spec: **`snh:1:<type>:<sha256>`** (spec already reserves `snh:` + w3id.org/soranoha — matches storage rec). **AMENDED (F83, round 15): drop the date from release names — canonical identity = the full typed manifest id, dates presentation-only — because the Git committer timestamp is UNSIGNED: the same signed manifest could acquire a different derived name when repackaged. This reverses the date-inferable-name preference the owner stated at D3. RATIFIED by owner 2026-08-25 (round 16); the `r<hash12>` display convention is presentation outside the protocol.** | owner, 2026-08-24; F83 amendment proposed | prefix trivially renameable pre-first-release; frozen after |
