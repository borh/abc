...
`sha256-rfc8785-safe-integer-domain-abc-v1`.
...
`corpus_generation_ref = sha256(rfc8785-safe-integer-json-bytes-v1(index -
corpus_generation_ref))`; the aggregate copies that exact reference and the
algorithm ID `sha256-rfc8785-safe-integer-domain-abc-v1`. This excludes only the self-reference
