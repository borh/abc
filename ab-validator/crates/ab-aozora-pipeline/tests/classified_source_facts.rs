...
#[test]
fn rewritten_tortoise_emits_one_recovery_fact_for_map_edit_identity() {
    ...
    assert_eq!(recovered.len(), 1, "one map edit must produce one fact");
    assert_eq!(
...
#[test]
fn rewritten_tortoise_split_by_newline_emits_one_recovery_fact() {
    let source = "〔a`\nb〕";
    let recovered = lex(source)
        .classified_source_facts
        .into_iter()
        .filter(|fact| fact.construct_id == ConstructId::RecoveredVerbatim)
        .collect::<Vec<_>>();
    ...
    assert_eq!(
        recovered[0].source_span,
        ab_aozora_pipeline::Span::new(0, 10)
    );
}

#[test]
fn rewritten_tortoise_with_recognized_child_emits_one_recovery_fact() {
    let source = "〔a`｜青梅《おうめ》〕";
    ...
    assert_eq!(
        recovered[0].source_span,
        ab_aozora_pipeline::Span::new(0, 32)
    );
    assert!(
        facts
            .iter()
            .any(|fact| fact.construct_id == ConstructId::Ruby)
    );
}

#[test]
fn literal_tortoise_keeps_classifier_provenance_boundaries() {
    let facts = lex("〔literal〕").classified_source_facts;
    let recovered = facts
        .iter()
        .filter(|fact| fact.construct_id == ConstructId::RecoveredVerbatim)
        .collect::<Vec<_>>();
    ...
    assert_eq!(
        recovered[0].source_span,
        ab_aozora_pipeline::Span::new(0, 3)
    );
    assert_eq!(
        recovered[1].source_span,
        ab_aozora_pipeline::Span::new(10, 13)
    );
}
