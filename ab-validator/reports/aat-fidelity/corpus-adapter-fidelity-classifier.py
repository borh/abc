...
P = {
  ...
  "gaiji.unicode_codepoint":lambda n: n.get("kind")=="gaiji" and has(n,"resolved") and not has(n,"jis_code"),
  "gaiji.un_embed":        lambda n: n.get("kind")=="gaiji" and has(n,"description") and not has(n,"jis_code") and not has(n,"resolved"),
  "iteration.kunoji":      lambda n: n.get("kind")=="gaiji" and ("くの字" in (n.get("description") or "") or "〳" in (n.get("resolved") or "")),
  ...
  "decoration.bold_italic":lambda n: n.get("kind")=="style" and n.get("style_type") in ("bold","italic"),
  "indentation.chitsuki":  lambda n: n.get("kind")=="style" and n.get("style_type")=="chitsuki",
  "indentation.burasage":  lambda n: n.get("kind")=="style" and n.get("style_type")=="burasage",
  "indentation.jizume":    lambda n: n.get("kind")=="style" and n.get("style_type")=="jizume",
  "indentation.jisage_oneline": lambda n: n.get("kind")=="style" and n.get("style_type")=="jisage_line",
...
def walk(v, ad):
    if isinstance(v, dict):
        for sid,pred in P.items():
            try:
                if pred(v): counts[ad][sid]+=1
            except Exception: pass
        for x in v.values(): walk(x, ad)
    elif isinstance(v, list):
        for x in v: walk(x, ad)

for ad in ("aozora2","aozora-rs"):
    n=0
    for f in glob.glob(f"{ROOT}/{ad}/aat/*/*.json"):
        try: walk(json.load(open(f)).get("blocks",[]), ad); n+=1
        except Exception: pass
...
def classify(rate, recognition):
    ...
    if rate < 0.05: return "dropped" if recognition!="aborts" else "not_applicable"
...
report=[]
for sid,pred in P.items():
    inp=inputs.get(sid,{}); den=inp.get("occ",0)
    ...
    for ad in ("aozora2","aozora-rs"):
        ...
        row["adapters"][ad]={"num":num,"rate":round(rate,3) if rate is not None else None,
                             "recognition":rec,"was":(inp.get("fidelity") or {}).get(ad),"now":cls}
    report.append(row)
report.sort(key=lambda r:-r["denominator"])
json.dump({"classified":report,"skipped":SKIP_REASON}, open(SP/"corpus-fidelity.json","w"), ensure_ascii=False, indent=1)

# sanity print
print(f"\n{'construct':26} {'den':>8}  aozora2(num rate was->now)      aozora-rs(num rate was->now)")
for r in report:
    a2=r["adapters"]["aozora2"]; rs=r["adapters"]["aozora-rs"]
    print(f"{r['construct']:26} {r['denominator']:>8}  a2:{a2['num']:>7} {str(a2['rate']):>6} {str(a2['was']):>10}->{str(a2['now'])!s:<11} rs:{rs['num']:>7} {str(rs['rate']):>6} {str(rs['was'])}->{rs['now']}")
