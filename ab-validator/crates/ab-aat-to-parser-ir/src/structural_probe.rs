...
pub fn render_markdown(summary: &StructuralProbeSummary) -> String {
    ...
    out.push_str("\n## Next Expansion\n\n");
    out.push_str("Use ABC's machine-readable generated workset export (`$AB_WORKSPACE_ROOT/abc/out/reports/tei-eaj-aozora/tei-eaj-aozora-workset-export.json`, or an explicit `AB_TEI_EAJ_WORKSET`) for the TEI-EAJ missing-counterpart expansion. The expansion should materialize structural evidence for those rows; it should not turn conversion compatibility evidence into a parser-selection claim.\n");
    out
}
