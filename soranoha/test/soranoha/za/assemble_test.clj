    ...
    (let [run-b (corpus/run-corpus! root store)
      ...
      (testing "(b) stages invalidated and executed"
        (let [executed (oracle/executed-stages run-b)]
          (is (= #{:extract :metadata :parse :convert :render-ir :render :plaintext :markdown :validate :accountability :coverage}
                 (executed (slug-of added-work)))
              "a new work executes the full chain")
          (is (= #{:extract :parse :convert :render-ir :render :plaintext :markdown :validate :accountability :coverage}
