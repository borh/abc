...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    let gold_path = std::env::var("ORTHO_GOLD_PATH")
        .expect("set ORTHO_GOLD_PATH to the labeled JSONL");
    ...
    while i <= 6 {
        ...
        i += 1;
        let det = HeuristicV1::new(tokenizer.clone(), HeuristicConfig {
        ...
        println!("  {:.2}    | {:.4}   {:.4}    {:.4}  | {} {} {} {}", t, recall, precision, f1, tp, fn_, fp, tn);
        ...
        if recall >= 0.85 && best.map_or(true, |b| f1 >= b.3) {
            best = Some((t, recall, precision, f1));
        }
    }
    println!();
    if let Some((t, r, p, f1)) = best {
        println!("BEST (recall >= 0.85, max F1): threshold={:.2} recall={:.4} precision={:.4} f1={:.4}", t, r, p, f1);
    } else {
        eprintln!("WARN: no threshold cleared recall >= 0.85 — escalation needed (see plan Task 4 EARLY EXIT B)");
    }
    Ok(())
}
