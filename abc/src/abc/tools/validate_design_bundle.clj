...
(def schemas
  "Domain-owned cross-event invariants for the design-bundle artifacts
...
(def ^:private design-fixture-inputs
  ["examples/ab-validator-output/comparison-report.json"
   ...
   "test/fixtures/parser-rq/publication-capture/identity.json"
   "test/fixtures/parser-rq/publication-capture/index.json"
   "test/fixtures/parser-rq/publication-capture/manifest.json"
   "test/fixtures/parser-rq/publication-capture/store/sha256/0e/0ec0f64b6eabe45ff4858d515a4ce7a067e166e64a042a342ece31b8a11152a9.json"
   "test/fixtures/parser-rq/publication-capture/store/sha256/57/571f89970cf7c2f13fae68db0b9b1f4665a53bbd8986cb417ff3636792995131.json"
   "test/fixtures/parser-rq/publication-capture/store/sha256/9b/9b970ba594d27c48cce4aa7abcb2f4818d09a0154c0c2d286c949e5aff91ba8f.json"
   "test/fixtures/parser-rq/publication-capture/store/sha256/bc/bcc00b7c0a77191e143c27db03d1b6a819599d1fa2e139c2a6a63b113679c014.json"
...
(defn validate-json-schemas! [extra-manifest-paths]
  (let [manifest-schema (files/read-json "schemas/manifest.schema.json")
        ...
        parser-rq-build-record-schema (files/read-json "schemas/parser-rq-build-record.schema.json")
    ...
    (doseq [[path schema] [["schemas/manifest.schema.json" manifest-schema]
                           ...
                           ["schemas/parser-rq-build-record.schema.json" parser-rq-build-record-schema]
    ...
    (let [policy (files/read-json "data/parser-rq-ab-aozora-classified-source-v1.json")
      ...
      (validate-json! parser-rq-classified-source-policy-schema
                      "data/parser-rq-ab-aozora-classified-source-v1.json")
      (check-errors! (classified-source/policy-errors policy))
    ...
    (let [root "test/fixtures/parser-rq/diagnostic-gap"
      ...
      (doseq [path ["result-available.json" "result-unavailable.json"]]
        (validate-json! parser-rq-diagnostic-gap-result-schema
                        (str root "/" path)))
      (check-errors! (diagnostic-gap/policy-errors policy))
      (check-errors! (diagnostic-gap/raw-diagnostics-errors
    ...
    (let [root "test/fixtures/parser-rq/source-recognition"
      ...
      (doseq [path ["aggregate-ok.json" "aggregate-unavailable.json"]]
        (validate-json! parser-rq-source-recognition-aggregate-schema
                        (str root "/" path)))
      (check-errors! (source-recognition/work-errors work))
      (check-errors! (source-recognition/index-errors index))
      (check-errors! (source-recognition/aggregate-errors aggregate))
      (check-errors! (source-recognition/coherence-errors
...
(defn validate-ab-validator-output! []
  (let [manifest-inputs (files/read-json (files/path "examples" "ab-validator-output" "manifest-inputs.json"))
    ...
    (am/explain-or-throw! ::manifest-inputs manifest-inputs
    ...
    (check-errors! (sentence-policy/paragraph-coherence-errors parser-ir))
    (check-errors! (sentence-policy/sentence-coherence-errors parser-ir))
