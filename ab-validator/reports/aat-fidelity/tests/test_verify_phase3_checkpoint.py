...
def audit(version):
    return {
        "mapping": {"mapping_version": "0.2.8", "mapping_hash": MAPPING_HASH},
...
def write_all(tmp_path, mutate=None):
    docs = {
        "s0p": summary(
            ...
            V0,
            bin_sha="0" * 64,
            details={"compared": 17886, "missing_count": 0, "bytes": {"diverged_count": 0}},
        ),
        "s0c": summary("stage0", "conformance", C0, V0, bin_sha="0" * 64),
        "s0f": summary("stage0", "perf", C0, V0, bin_sha="0" * 64, details={"new_timeouts": 0}),
        "rap": summary(
            "rotation-a",
            "delta",
            C1,
            ...
            details={
                ...
                "compared": 17886,
                "classes": {"identical": 17700, "rewritten": 186, "span_confined": 0},
                "verdict": "PASS",
            },
        ),
        "rac": summary(
        ...
        "raf": summary("rotation-a", "perf", C1, V1, bin_sha="1" * 64, details={"new_timeouts": 0}),
        "rbp": summary(
            "rotation-b",
            "confinement",
            C2,
        ...
        "rbf": summary("rotation-b", "perf", C2, V2, bin_sha="2" * 64, details={"new_timeouts": 0}),
        "auda": audit(V1),
        "audb": audit(V2),
    }
    if mutate:
...
def test_bin_mismatch_within_stage_rejected(tmp_path):
    def mutate(d):
        d["raf"]["candidate"]["bin_sha256"] = "9" * 64
...
def test_wrong_version_pattern_rejected(tmp_path):
    def mutate(d):
        d["rbp"]["candidate"]["version"] = V1.replace(C1, C2)
...
def test_delta_class_totals_must_sum_to_compared(tmp_path):
    def mutate(d):
        d["rap"]["details"]["classes"]["identical"] = 17000
...
def test_confinement_wrong_mode_rejected(tmp_path):
    def mutate(d):
        d["rbp"]["details"]["mode"] = "container-rewrite"
...
def test_audit_wrong_mapping_rejected(tmp_path):
    def mutate(d):
        d["auda"]["mapping"]["mapping_version"] = "0.2.4"
...
def test_audit_identity_mismatch_rejected(tmp_path):
    def mutate(d):
        d["auda"]["compatibility_candidates"][0]["aat_adapter_version"] = V2
