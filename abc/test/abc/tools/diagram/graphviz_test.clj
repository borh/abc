...
(deftest dot-is-stable-sorted-and-escaped
  (let [out (graphviz/dot sample)]
    ...
    (is (str/starts-with?
         out "// GENERATED by abc.tools.diagram.graphviz/dot — do not edit"))
...
(deftest canvas-defaults-to-black-with-explicit-transparent-override
  (testing "a declared canvas color is applied"
    (is (str/includes? (graphviz/dot sample) "bgcolor=\"#000000\"")))
  (testing "an absent canvas defaults to the black presentation background"
    (is (str/includes? (graphviz/dot (update sample :theme dissoc :canvas))
                       "bgcolor=\"#000000\"")))
  (testing "transparency requires the explicit :transparent override"
    (is (str/includes?
         (graphviz/dot (assoc-in sample [:theme :canvas] :transparent))
         "bgcolor=\"transparent\""))))

(deftest font-family-is-theme-driven-with-noto-default
  (testing "an absent :font-family falls back to Noto Sans CJK JP"
    (is (= 5 (count (re-seq #"fontname=\"Noto Sans CJK JP\""
                            (graphviz/dot sample))))
        "graph default, node default, one cluster, and two edges"))
  (testing "a declared :font-family reaches graph, node, cluster, and edge"
    (let [out (graphviz/dot (assoc-in sample [:theme :font-family]
                                      "Example Grotesk"))]
      (is (= 5 (count (re-seq #"fontname=\"Example Grotesk\"" out))))
      (is (not (str/includes? out "Noto Sans CJK JP"))))))
