...
1. **Soranoha workflow protocol** owns stable step records, provenance,
   cache-key construction, workflow reports, and schema compatibility.
...
- mature `:sync.lazy`, `:core-async.lazy-scheduling`, and
  `:applicative.virtual-futures` engines.

Nodely 2.0.3 is published as `dev.nu/nodely` and, since the 2.0.0 line,
`core.async`, `promesa`, and `manifold` are no longer provided transitively.
If Soranoha uses an engine that needs one of those libraries, Soranoha must
...
- `:executor` record: `:clojure`, `:nix`, `:shell`, or `:nodely`;
...
- record realized and skipped dependency keys;
...
+ node_kind
+ graph_version
+ stable leaf implementation hash
...
The engine family is included because changing from `:sync.lazy` to an async
engine can change observable timing, exception wrapping, and resource behavior.
It must not change artifact bytes for accepted release outputs; if it does,
that is a bug or a nondeterminism finding.

### Output Validity

A cached node result is valid only when:

- all declared output paths exist, if paths are declared;
- output content hashes match;
...
- Run `checked-env` in tests and development validation, not hot production
  paths.
- Do not add `core.async`, `promesa`, or `manifold` just to evaluate the first
  slice.
...
- Consider `:applicative.virtual-futures` only after the ABC JVM baseline is
...
- Verify `checked-env` behavior in tests.
- Do not call Nix.
...
**Acceptance check:** Accept the facade only if callers can understand and
test graph behavior without knowing Nodely internals.
...
- How much of a Nodely graph can be hashed stably when nodes contain function
  ...
  leaf implementation hashes, not raw function values.
