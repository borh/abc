...
out_dir="${AB_DB_ROOT:-$repo_root/scratch/state}/aat-fidelity/cross-adapter-smoke"
...
rg -n '"adapter": "aozora2"|"adapter": "aozora-rs"|"adapter": "aozora2html"|"adapter": "aozora"' "$out_dir/report.json"
rg -n '"schema_status"|"upstream_status"|"oracle_status"' "$out_dir/report.json"
rg -n 'Cross-Adapter AAT Oracle Summary|aozora2|aozora-rs|aozora2html|aozora' "$summary_md"
...
"$duckdb_bin" -csv -noheader "$out_dir/fidelity.duckdb" \
  "select count(distinct adapter) from fidelity_rows where adapter in ('aozora2', 'aozora-rs', 'aozora2html', 'aozora')" \
  | rg -n '^4$'
