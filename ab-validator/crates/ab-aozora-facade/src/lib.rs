...
/// let cst = ab_aozora_facade::cst::from_tree(&doc.parse());
/// // Walk the rowan SyntaxNode tree …
/// ```
// lifted-code note (Task 3): `aozora-cst` is not in the Task-2-confirmed
// LIFT_SET (feature-gated off by default upstream; excluded per the
// minimal-core decision — see docs/handoffs/2026-07-10-parser-fork-provenance.md).
// `cst` is declared as an empty stub feature in Cargo.toml (matching
// upstream's feature name, without its now-absent dependency) so this cfg
// gate resolves cleanly; enabling it would fail to compile, honestly
// reflecting that aozora-cst is not carried by this fork.
#[cfg(feature = "cst")]
...
/// ```rust,ignore
/// use ab_aozora_facade::Document;
/// use ab_aozora_facade::query::compile;
///
/// let doc = Document::new("｜青梅《おうめ》");
/// let cst = ab_aozora_facade::cst::from_tree(&doc.parse());
...
/// ```
// lifted-code note (Task 3): `aozora-query` is not in the Task-2-confirmed
// LIFT_SET (feature-gated off by default upstream; excluded per the
// minimal-core decision — see docs/handoffs/2026-07-10-parser-fork-provenance.md).
// `query` is declared as an empty stub feature in Cargo.toml (matching
// upstream's feature name, without its now-absent dependency) so this cfg
// gate resolves cleanly; enabling it would fail to compile, honestly
// reflecting that aozora-query is not carried by this fork.
#[cfg(feature = "query")]
