...
- **ADR-0002-C1 — structural-invariant:** The parser evidence index binds the three explicitly named July 2026 historical reports to their workspace-relative paths and current byte hashes, and does not identify them as the preregistered neutral comparison. Evidence boundary: `test/abc/tools/parser_evidence_test.clj`.
- **ADR-0002-C2 — structural-invariant:** Parser citation classes do not admit or release a parser; exact adapter, mapping, parser-IR schema, and evidence-scope equality through ADR 0023 is required for admission. Evidence boundary: `test/abc/tools/parser_evidence_test.clj` and `test/abc/tools/aat_parser_ir_compat_test.clj`.
- **ADR-0002-C3 — fixture-behavior:** The committed imported producer fixture carries parser IR, warning diagnostics, run-summary aggregation, mapping provenance, and schema identities accepted by the ABC file boundary. Evidence boundary: `test/abc/tools/parser_import_boundary_evidence_test.clj`.
- **ADR-0002-C4 — structural-invariant:** Changing the parser/adapter/mapping tuple changes parser derivation identity without changing the source relations `work_content_hash = bundle_hash` and independently checked `primary_text_hash`. Evidence boundary: `test/abc/tools/parser_identity_relations_evidence_test.clj`.
