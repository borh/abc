(ns ab-research.parser-rq-source-accountability-test
  (:require [ab-research.files :as files]
            ...
            [ab-research.parser-rq-source-accountability :as rq-source]
...
(defn- publish-json!
  [root manifest value]
  (let [text (record-json/write-deterministic-json-str value)
...
(deftest published-v1-evidence-still-validates-against-the-frozen-schemas
  (let [declared (fn [file]
                   (try (get (record-json/read-json-file file) "schema_version")
    ...
    (doseq [[wire [schema-path expected-count]] p1-v1-wire-schemas
      ...
      (doseq [file files]
        (is (nil? (schema/validation-errors schema (record-json/read-json-file file)))
...
(deftest every-region-and-metadata-number-stays-in-the-safe-integer-domain
  ...
  (let [beyond 9007199254740992
        ...
        work (record-json/read-json-file
              (io/file "test/fixtures/parser-rq/source-recognition/work-ok.json"))
        aggregate (record-json/read-json-file
