...
Coverage requires a 底本 colophon and either a separator preamble or a two-line title/author header. It includes basic ruby, Latin accent notation, numeric gaiji references from both JIS planes, correction notes, single-line historical double-angle quotes, non-overlapping retrospective emphasis dots, and the supported layout forms. Blank-line spacing and title/author metadata are not certified. Unsupported sources must not be read as passing. The report is internal review evidence, not a new signed publication artifact or publication gate; the existing include-and-flag validation policy is unchanged.
