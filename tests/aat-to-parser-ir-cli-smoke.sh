...
jq -e '.derived_from.mapping_version == "0.2.4"' "$out_dir/parser-ir.json"
jq -e '.derived_from.mapping_schema_hash == "sha256:23a2822cbae88533168121e8a09648441276d8af6484269ae666b90030eb1e06"' "$out_dir/parser-ir.json"
jq -e '.paragraphs[0].node_range == {"start":0,"end":2}' "$out_dir/parser-ir.json"
jq -e '.paragraphs[0].role == "body"' "$out_dir/parser-ir.json"
jq -e '.mapping.mapping_version == "0.2.4"' "$out_dir/divergence.json"
