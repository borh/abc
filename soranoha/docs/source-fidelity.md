...
TEI encodes paragraph-leading fullwidth indentation as `style="text-indent: 1em"` (scaled to the source count), and heading or source-note continuation indentation as `padding-inline-start`. Its `styleDefDecl` declares CSS. Source-note lines use `seg` and `lb`. Interior spaces remain text. Compact XML serialization prevents pretty-printing from inserting lexical whitespace around mixed content such as ruby. A viewer may format the XML tree without modifying the stored text.
