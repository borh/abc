(ns ab-research.json-test
  (:require [ab-research.json :as abc-json]
...
(deftest write-deterministic-json-str-matches-file-writer-test
  (let [value {"b" 2 "a" [3 1] "nested" {"z" 1 "y" 2}}
    ...
    (try
      ...
      (is (= (str (record-json/write-deterministic-json-str value) "\n")
             (slurp file)))
      (finally
        (.delete (io/file file))))))
...
(deftest write-deterministic-json-file-is-atomic-under-concurrency-test
  (let [dir (java.io.File/createTempFile "abc-json-atomic" "")
        ...
        expected #{(record-json/write-deterministic-json-str value-a)
                   (record-json/write-deterministic-json-str value-b)}
        stop (promise)
