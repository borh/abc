...
- **Attempt 2 (PASSED):** auto-jobs 19 (recalibrated budget; MemAvailable 60.7 GiB). Wall 59:43, peak RSS 65,620,620 kB, zero earlyoom events; startup sweep reclaimed attempt 1's orphaned staging (first production exercise). Row-count verification vs `full-2026-07-05_114245-jobs10`: analyses 71,540, morphemes 662,984,226, nway_regions 161,142,784, nway_feature_diffs 23,356,986,673, sources 17,885 — all exact, per-analyzer counts identical. Canonical swap executed; `full-2026-07-05_164518-jobs0` is canonical.
- **Honest wall-clock finding:** jobs=19 (60 min) is SLOWER than the pre-Arc jobs=10 run (42 min), and peak RSS (65.6 GB) far exceeds the linear model's prediction (~41 GB) — full-corpus memory is dominated by the large-document tail, roughly independent of job count beyond ~10. Conclusions: (a) reliability goal met — unattended completion, safe failure, self-healing staging; (b) the "more jobs via freed memory" speed theory is refuted on this box — wall-clock optimum is at LOWER job counts (page-cache pressure at high RSS costs more than parallelism gains); the deferred §3.15 (intra-worker parallelism / work-stealing) is the real speed lever, and a small jobs-vs-wall sweep (e.g. 8/10/12/16) would find the practical optimum cheaply.
