...
SIG = {
    ...
    "decoration.bousen": {
        ...
        "note": "aozora folds into `emphasis` marker",
    },
    "heading.basic": {
        "kinds": {"heading"},
    ...
    "indentation.jizume": {
        "styles": {"jizume"},
        "markers": {"jizume", "lineJizume"},
        "note": "字詰め chars-per-line",
    },
    "decoration.bold_italic": {
        "styles": {"bold", "italic"},
        "markers": {"lineBold", "lineItalic"},
        "note": "aozora emits only lineBold (~drops bold/italic)",
    },
    "warichu.basic": {
        "kinds": {"warigaki"},
        "styles": {"warichu", "warigaki"},
        "markers": {"warigaki", "marginNote"},
        "note": "割注 inline double-line annotation",
    },
    "layout.yokogumi": {
        "kinds": {"yokogumi", "yokogumi_block"},
        "styles": {"yokogumi"},
...
def main():
    ...
    # human matrix
    print(
        f"\n{'construct':26}{'denom':>8}  " + "".join(f"{a[:9]:>10}" for a in ORDER),
        file=sys.stderr,
    )
    for r in rows:
        cells = "".join(
