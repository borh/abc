...
enum Command {
    AnalyzeAat {
        ...
        #[arg(
            ...
            help = "Orthographic normalization before analysis.

[off] Preserve source text (default).
[heuristic] Character-pattern and proper-noun detection for katakana prose.
[ml] Character-feature logistic regression; requires --ortho-ml-model.
Model hashes identify bytes, not training provenance or accuracy on new authors.
[historical] Normalize historical kana using the kindai-bungo oracle. All
analyzers receive the same normalized text. Select old-kana works with
--works-parquet and --orthographic-style 新字旧仮名,旧字旧仮名."
        )]
