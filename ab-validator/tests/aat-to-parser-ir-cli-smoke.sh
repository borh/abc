...
jq -e --slurpfile mapping "$repo_root/data/aat-to-parser-ir-mapping-v1.json" '.derived_from.mapping_version == $mapping[0].mapping_version' "$out_dir/parser-ir.json"
...
jq -e --slurpfile mapping "$repo_root/data/aat-to-parser-ir-mapping-v1.json" '.mapping.mapping_version == $mapping[0].mapping_version' "$out_dir/divergence.json"
...
jq -e '.inputs[0].parser_ir.paragraph_count == 2' "$out_dir/structural-summary.json"
jq -e '.inputs[0].parser_ir.paragraphs_represented == true' "$out_dir/structural-summary.json"
...
jq -e '.totals.parser_ir_gap_rows == 0' "$out_dir/tei-eaj-structural-summary.json"
jq -e '.totals.source_attribution_gap_rows == 0' "$out_dir/tei-eaj-structural-summary.json"
