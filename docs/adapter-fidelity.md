...
| `aozora2` | Direct incomplete | `aozora-core` 0.7.1 from crates.io | `aozora_core::tokenize` then `aozora_core::parse` | Text, ruby with nested content, gaiji, accent, figure, warigaki, inline wrappers, notes, and reconstructed supported block containers from `BlockStart`/`BlockEnd` for jisage, keigakomi, yokogumi, and caption. | Block fidelity is still incomplete for unsupported block boundary types such as chitsuki, jizume, burasage, and block tcy, which remain raw. `--mode html` is unsupported because `aozora-core` does not expose an upstream HTML renderer. |
...
- Add an oracle table for expected gaiji resolutions independent of adapter
  output.
- Expand `aozora2` block reconstruction to the remaining upstream block types.
