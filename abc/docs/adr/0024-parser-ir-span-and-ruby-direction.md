...
- **ADR-0024-C1 — fixture-behavior:** The current parser-IR schema accepts `decoded_utf8` spans and ruby direction values `left`, `right`, and null, and rejects an unknown coordinate system. See `test/abc/tools/parser_ir_schema_evidence_test.clj`.
- **ADR-0024-C2 — fixture-behavior:** The parser-IR span definition continues to accept the legacy `{start,end}` and `{start,end,line,column}` object shapes. See `test/abc/tools/parser_ir_schema_evidence_test.clj`.
- **ADR-0024-C3 — fixture-behavior:** Current publication fixtures carry and consume `decoded_utf8` spans. See `test/abc/tools/parser_ir_schema_evidence_test.clj`.
- **ADR-0024-C4 — fixture-behavior:** TEI rendering preserves ruby direction using a profile-valid `rend` value. See `test/abc/tools/parser_ir_tei_test.clj`.
- **ADR-0024-C5 — structural-invariant:** Parser-IR schema JCS hash changes propagate to the parser-IR schema coordinate copied into materialized manifests. See `test/abc/tools/parser_ir_schema_evidence_test.clj`.
