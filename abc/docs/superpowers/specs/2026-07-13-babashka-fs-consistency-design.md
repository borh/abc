...
Approved and review-amended for detailed planning on 2026-07-13.
...
calls. The important compound semantics are already named by `abc.tools.files`; an
individual `.exists` call is not inherently more complected than `fs/exists?`. The
remaining problem is idiom uniformity: new code copies whichever local form it
encounters, edge handling diverges over time, and reviewers must recognize several
equivalent vocabularies. This migration stops that future divergence and makes the
house style enforceable. Its consistency benefit must be weighed honestly against the
regression risk of touching many stable call sites.
...
Drift prevention ships before legacy migration. The initial policy contains a checked-
in grandfather set of existing production namespaces and exact low-level exceptions.
Unlisted/new namespaces must comply immediately. Each migration batch removes entries;
the policy test rejects additions to the grandfather set, so it can only shrink from
its committed baseline. This decouples the high-value guard against new divergence from
the riskier legacy rewrite.
...
| --- | --- | --- |
| `.exists` | `fs/exists?` | False for missing and inaccessible paths, matching the underlying existence-probe behavior |
...
- Atomic publication using `Files/createTempFile`, `Files/move` with `ATOMIC_MOVE`
  and `REPLACE_EXISTING`, and `finally`/`Files.deleteIfExists` cleanup. In particular,
  the coupled temp/move/cleanup paths in `abc.tools.json` and
  `abc.tools.diagram.presentation-registry` remain explicit units.
- Staged archive validation in `abc.tools.source-bundle`, where
  `Files/createTempFile` and `finally` cleanup are part of the failure contract.
...
1. Enforcement baseline: add the syntax-aware policy and the fixed grandfather set.
   Prove that new violations fail while all existing legacy entries are named. Commit
   this independently so drift prevention lands before legacy churn.
2. Predicates and directory creation: migrate existence/type checks and `mkdirs`, with
   characterization for missing paths, symlinked files/directories, and idempotence.
3. Directory listing and recursive traversal: migrate `listFiles` and `file-seq`,
   preserving missing-directory results, filters, relative paths, and explicit sorting.
   Adding explicit sort keys here is a genuine decomplection: it replaces today's
   dependence on unspecified directory enumeration order with a named contract.
4. Path manipulation: migrate canonicalization, absolutization, parents, filenames,
   and relativization where no Java boundary requires a `File`.
5. Mutation: migrate ordinary copies, non-atomic moves, and deletion. Temporary files
   migrate only where `fs/with-temp-dir` has demonstrably equivalent ownership and
   cleanup timing; the named atomic-publication and staged-archive temp paths are out of
   scope and remain documented exceptions.
6. Enforcement cleanup: remove unused imports and grandfather entries, confirm the
   grandfather set is empty apart from permanent named exceptions, and verify the
   complete repository.

The batches may touch the same namespace. They are organized around contracts and are
reviewable in isolation; interleaved edits mean a later single-batch revert is not
promised.
...
This creates a permanent representation seam: Path internally, File at Java/library
boundaries. The conversion tax is accepted in exchange for one filesystem vocabulary.
Conversions happen at the last responsible moment: `fs/file` only when the consumer
requires File, and `str` only when a serialized, CLI, process, or exception contract
requires text. Repeated Path/File oscillation inside a function is a failed migration,
not an acceptable style.
...
Characterization precedes each behavior batch and runs against the old implementation
before production edits. Those same tests run unchanged after migration. Only this
old-green/new-green sequence is evidence of preservation; tests first introduced after
the replacement do not establish equivalence. Focused golden tests cover:
...
The source policy parses production `.clj` and `.cljc` forms rather than searching raw
substrings. It recognizes Clojure's host-interop spellings, including `(.exists x)`,
`(. x exists)`, and thread/dot forms, without matching comments or string literals.
Because Clojure source does not always reveal a receiver's runtime class, ambiguous
method names such as `delete` and `exists` use exact form/site exceptions for known
non-File receivers; a broad token ban is forbidden. The matcher has sensitivity tests
for every supported interop spelling and collision tests for non-filesystem receivers.
An AST/source-form policy test is preferred here over a clj-kondo analyze-call hook:
...
empty at completion apart from permanent exact exceptions.
