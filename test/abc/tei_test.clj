...
(defn tei-fixture [f]
  (let [work-record (xtdb/work-query "文鳥" "夏目")]
    ...
    (binding [*metadata* (metadata-to-tei work-record) #_(mg/generate [:schema {:registry registry} :tei/header])
              *text* (abc.annotation/parse-text "A［＃B］C｜D《E》F※［＃1-86-29］G［H］I｜J《K》L") #_(mg/generate [:schema {:registry abc.annotation/registry} :document/body])]
      (println *text*)
      (f))))
...
(deftest header-test
  ...
  (let [h (header *metadata*)]
    (is (schema-valid
          ...
          h
          (merge registry
                 abc.annotation/registry)))))

(deftest body-test
  (is *text*)
  (is (gen/sample (mg/generator [:schema {:registry abc.annotation/registry} :document/body])))
  (let [b (body *text*)]
