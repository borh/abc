...
| Full requalification | Operator starts a new qualification from a clean Linux/cgroup-v2 host within one working day, from a documented runbook | `docs/parser-rq-runbook.md` (Recipe 2), `bin/parser-rq-campaign-capture.sh`, `config/parser-rq-site.example.json` | **met — host-exercised 2026-07-25**, well inside one working day (~15 min of command time; the capture itself 51 s on the pinned corpus). Ran end to end to `verify-promotion` → `ok`, minting candidate `sha256:ae9a97bf…` with `gate_status: release-qualified` (9/9 `pass`). The draft recipe was **not literally followable**: three steps were missing entirely (realize/compare provenance, `bind-provenance`, blobs + `verify-evidence` receipt, run-root assembly), two provenance artifacts shared one filename, `--evidence-tree` was undefined and is not the checkout, and `admission-candidates` had no producer and is EDN not JSON. All corrections are folded into Recipe 2 and marked **[corrected]**. Caveat: `diagnostic-completeness` passes **vacuously** on the 3-work pinned corpus (`diagnostic_count: 0`) |
