...
(deftest adr-0034-c1-contract
    ...
    (doseq [key [:observed :inputs :verdict]]
      (is (contains?
           (kinds (registry-problems
                   ...
                   (assoc (entry :structural-invariant :structural-test)
                          key true)
  ...
  (testing "artifact identity and current inputs are independently bound"
    (let [repo (fs/create-temp-dir {:prefix "adr-0034-c1-"})
          input (fs/file repo "input.txt")
          artifact-path "docs/evidence/run.json"
      ...
      (try
        ...
        (let [input-hash (hash/format-sha256 (hash/sha256-file input))
          ...
          (is (= #{:artifact-hash-mismatch}
                 (kinds (:problems
                         (bundle/validate-bundle repo artifact-path
                                                 (str "sha256:" (apply str (repeat 64 "f")))
                                                 ["ADR-0034-C1"])))))
          (spit input "two")
          (is (contains?
               (kinds (:problems (bundle/validate-bundle repo artifact-path canonical
                                                         ["ADR-0034-C1"])))
               :input-hash-mismatch)))
...
(deftest adr-0034-c2-contract
  (testing "lifecycle vocabulary, complete closure, cycles, and shortest witnesses"
    (let [dir (fs/create-temp-dir {:prefix "adr-0034-c2-"})]
      (try
        (doseq [[name body]
          ...
          (spit (fs/file dir name) body))
        (fs/create-dirs (fs/file dir "test"))
        (spit (fs/file dir "test/x.clj") "(ns x)")
